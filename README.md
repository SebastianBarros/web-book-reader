...
- **Audiobook mode.** Press the play icon in the top bar and the book is read aloud. Two providers: **Cloud** (Google Cloud Text-to-Speech Chirp-HD / Neural2 voices via a free Cloudflare Worker proxy — [details](cloudflare-google-tts.md) — Spanish voices curated, default is `es-US-Chirp-HD-F`) and **Browser** (your OS's built-in voices, offline). Cloud is the default for quality; switch to Browser in Settings if you're offline. Auto-turns pages, advances across chapters, prefetches the next 5 paragraphs so there's no gap between blocks. Tap any paragraph to start reading from there. Retries transient fetch errors 3× with exponential backoff; on permanent failure it stops at that paragraph with a clear toast. Zero API keys in the client.
- **Stop at end of chapter.** Moon icon in the top bar during playback — arm it and the audiobook pauses automatically when it crosses into the next TOC chapter. Works for both MOBI and EPUB, driven by the actual chapter anchors in the rendered DOM rather than the paginator's page fraction, so it fires at the real boundary rather than some nearby page turn.
